...
- In-game control overlay with at least ten trick combinations using `A` or `S` plus directional input.
...
2. Run the game locally:
   ...
   ```
 3. For web hosting (e.g. Vercel), ensure `vercel.json` points to `index.html` and deploy the static files. The demo jar is stored as `HalfPipeGame.jar.base64` and reconstructed by the page on launch.

## Testing
- Verify the project compiles with `./gradlew build`.
- Launch the game and confirm:
  - Menu scene appears and `Space` or gamepad `Start` loads the game.
  - Skater moves with arrow keys or left stick.
  - Performing the listed key combinations with `A` or `S` displays the corresponding trick names.
